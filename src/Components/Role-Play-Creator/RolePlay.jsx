...
const RolePlay = () => {
  ...
  const validateOther = () => {
    ...
    if (/^\d/.test(otherIndustry.trim()) || otherIndustry.trim().length < 2) {
      setOtherError('Please add valid industry');
      setIsOtherValid(false);
      return false;
    }
    setOtherError('');
  ...
  const validateLearningObjective = () => {
    ...
    if (/^\d/.test(learningObjective.trim()) || learningObjective.trim().length < 2) {
      setLearningObjectiveError('Please add valid learning objective');
      setIsLearningObjectValid(false);
      return false;
    }
            ...
            <Box sx={{ display: 'flex', gap: 2, marginBottom: 0 }}>
              ...
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  sx={{ height: '50px', '.MuiOutlinedInput-input': { height: '25px' } }} // Adjust the height here
                  MenuProps={{
                    MenuListProps: {
                      style: {
                        color: 'rgb(97,97,97)', // Change text color
