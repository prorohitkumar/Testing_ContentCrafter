...
const DocChat = () => {
    ...
    useEffect(() => {
      ...
      const handleBeforeUnload = async (event) => {
        ...
        try {
          ...
          await fetch(`https://document-quest.onrender.com/delete-all-files/${session_id}`, {
            method: 'DELETE'
          });
        } catch (error) {
    ...
    const fetchData = async () => {
      try {
        const session_id = localStorage.getItem("session_id");
        const response = await fetch(`https://document-quest.onrender.com/files/${session_id}`);
    ...
    const handleUploadAgree = async () => {
      ...
      try {
        const session_id = localStorage.getItem("session_id");
        const response = await fetch(`https://document-quest.onrender.com/upload/${session_id}`, {
          method: 'POST',
          body: formData
        });
        if (response.ok) {
    ...
    const handleDeleteAgree = async () => {
      try {
        ...
        const response = await fetch(`https://document-quest.onrender.com/delete_file/${session_id}?filename=${documentToDelete}`, {
          method: 'DELETE'
        });
        if (response.ok) {
    ...
    const handleEndSessionAgree = async () => {
      try {
        const session_id = localStorage.getItem("session_id");
        const response = await fetch(`https://document-quest.onrender.com/delete-all-files/${session_id}`, {
          method: 'DELETE'
        });
        if (response.ok) {
          console.log('File deleted successfully');
    ...
    return (
      ...
      >
        ...
        <DialogActions>
          ...
          <Button disabled={uploadDisabled} onClick={handleUploadAgree} autoFocus>
            Upload
          </Button>
        </DialogActions>
      </Dialog>
      ...
      >
        <DialogTitle id="alert-dialog-title">
          {"Do you want to exit 'Ask Document'?"}
        </DialogTitle>
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            Note: Exiting 'Ask Document' will delete all documents and indexing from ContentCrafter.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
